...
# part a)
# read_puzzle(fname) reads information from fname file and 
#returns the info as 
# Puzzle value.
...
def  read_puzzle(filename):
    ...
    l = f.readlines()
    size = int(l[0])
    board = list(map(lambda x: x.split(), l[1:size+1]))
    constraints = list(map(lambda y: y.split(), l[size+1:]))
    for i in constraints:
        i[1] = int(i[1])
    p = Puzzle(size, board, constraints)
    f.close()
    return p    
...
# space in puz, or False if no cells are blank.  If the first 
# constraint has only guesses on the board, find_blank 
# returns 'guess'.  
...
check.expect("Tc4", find_blank(Puzzle(3, [["b","c","a"],["a","a","a"],["d","a","e"]],[["a",18,"*"],["b",1,"="],["c",2,"="],["d",3,"="],["e",1,"="]])), Posn(2,0))
check.expect("Tc5",find_blank(Puzzle(3, [[1,2,"d"],[Guess("b",2),3,Guess("b",1)],[Guess("b",3),Guess("b",1),Guess("b",2)]],[["b",12,"*"],["d",3,"="]])), 'guess')

# part d)
# used_in_row(puz, pos) returns a list of numbers used in the 
# same row as (x,y) position, pos, in the given puz.  
...
def used_in_row(puz, pos):
    ...
    for i in c:
        if isinstance(i, int):
            d = d + [i]
        elif isinstance(i, Guess):
            d = d + [i.number]
    return sorted(d)

check.expect("Td1", used_in_row(puzzle1, Posn(1,1)), [])
check.expect("Td2", used_in_row(puzzle1partial2, Posn(0,1)),
             [1,2,4])
check.expect("Td3", used_in_row(Puzzle(3,[[1,2,'d'],['b',3,'b'],
['b','b','b']],[['b',12,'*'],['d',3,'=']]), Posn(0,1)), [3])

# used_in_col(puz, pos) returns a list of numbers used in the 
# same column as (x,y) position, pos, in the given puz.  
...
def used_in_col(puz, pos):
    ...
    for k in c:
        if isinstance(k, int):
            d = d + [k]
        elif isinstance(k, Guess):
            d = d + [k.number]
    return sorted(d)

check.expect("Td4", used_in_col(puzzle1partial2,Posn(1,0)), 
             [2,3])  
check.expect("Td5", used_in_col(puzzle2soln,Posn(3,5)), 
             [1,2,3,4,5,6])  

# part e)
# available_vals(puz,pos) returns a list of valid entries for 
# the (x,y) position, pos, of the consumed puzzle, puz.  
...
def available_vals(puz, pos):
    a = used_in_row(puz, pos)
    b = used_in_col(puz, pos)
    c = a + b
    d = len(puz.board)
    e = []
    i = 1
...
check.expect("Te1", available_vals(puzzle1partial,
                                   Posn(2,2)), [2,4])
check.expect("Te2", available_vals(puzzle1partial2, 
                                   Posn(0,1)), [3])
...
# part h) 
# apply_guess(puz) converts all guesses in puz into their corresponding 
# numbers and removes the first contraint from puz's list of contraints
...
def neighbours(puz):
    ...
    if find_blank(puz) == False:
        ...
        b = find_blank(puz)
        c = available_vals(puz, b)
        a = a + list(map(lambda x: fill_in_guess(puz, b, x), c))
    return a
